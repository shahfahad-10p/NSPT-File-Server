...
if (!fs.existsSync(global.config.rootFolder)) {
    fs.mkdir(global.config.folder, (err) => { });
}
...
router.post('/upload', function (req, res, next) {
    ...
    fs.writeFile(path.join(__dirname, global.config.rootFolder, req.body.fileName), base64Image, { encoding: 'base64' }, (err) => {
        if (err) {
...
router.post('/download', function (req, res, next) {
    filepath = path.join(__dirname, global.config.rootFolder) + '/' + req.body.fileName;
